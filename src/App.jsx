// src/App.jsx — ФИНАЛЬНО РАБОЧАЯ ВЕРСИЯ
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-amber-100 z-50">
      <div className="flex justify-around items-center max-w-md mx-auto py-3">
        <Link to="/" className={`px-6 py-2 font-medium ${location.pathname === '/' ? 'text-amber-700 font-bold' : 'text-amber-600'}`}>
          Главная
        </Link>
        <Link to="/district/Chakvi" className={`px-6 py-2 font-medium ${location.pathname.startsWith('/district') ? 'text-amber-700 font-bold' : 'text-amber-600'}`}>
          Районы
        </Link>
        <Link to="/calculator" className={`px-6 py-2 font-medium ${location.pathname === '/calculator' ? 'text-amber-700 font-bold' : 'text-amber-600'}`}>
          Калькулятор
        </Link>
      </div>
    </div>
