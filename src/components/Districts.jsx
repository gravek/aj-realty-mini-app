...
const getPriceCategories = (district) => {
  ...
  return (
    <div className="mt-6 pb-32 relative">
      ...
      {/* Основной контент */}
      <div className="max-w-6xl mx-auto ml-10 px-4">
        {/* Заголовок страницы */}
            ...
            <section key={key} id={`district-${key}`} className="relative py-8">
              ...
              <div className="mb-12">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-2xl font-bold text-${colors.text}-900">Почему {district.name}?</h3>
                  <div className="flex gap-2">
                    <button
                      onClick={() => setActiveAdvantageTab('lifestyle')}
                      className={`px-4 py-2 rounded-lg transition ${activeAdvantageTab === 'lifestyle' ? 'bg-cyan-100 text-cyan-700 font-medium' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                    >
                      Для жизни
                    </button>
                    <button
                      onClick={() => setActiveAdvantageTab('investment')}
                      className={`px-4 py-2 rounded-lg transition ${activeAdvantageTab === 'investment' ? 'bg-emerald-100 text-emerald-700 font-medium' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                    >
                      Для инвестиций
                    </button>
                    <button
                      onClick={() => setActiveAdvantageTab('nature')}
                      className={`px-4 py-2 rounded-lg transition ${activeAdvantageTab === 'nature' ? 'bg-teal-100 text-teal-700 font-medium' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                    >
                      Природа
                    </button>
                  </div>
